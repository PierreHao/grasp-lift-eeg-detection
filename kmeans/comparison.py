...
def mykmeans_plus_plus(X, max_clusters = 8, maxiter=10000):
    ...
    for nIter in range(0, maxiter):
        ...
        if (notImproveCount>10):
            print "(plusplus) Last iteration: ", nIter, " inertia: ",inertia, " minInertia: ",minInertia   
            break
...
def onlineKmeans(X,max_clusters=3,b=30,maxiter=1000):
    ...
    for i in range(maxiter):
        ...
        if (minInertia ==0 or inertia < minInertia):
            ...
            notImproveCount+=1
        if (notImproveCount>10):
            print "(Online) Last iteration : ", i, ", inertia: ",inertia, ", minInertia:",minInertia
            break
...
inputSizesToGenerate = [[numberOfSamples, 2**2],[numberOfSamples, 2**4],[numberOfSamples, 2**6],[numberOfSamples, 2**8],[numberOfSamples, 2**10],[numberOfSamples, 2**12],[numberOfSamples, 2**14]]
...
#         myfile.write("numSamples: "+numSamples)
#===============================================================================

for num_samples, num_cluster in inputSizesToGenerate:
    print "Running for {0} samples of dimension {1} and nCluster {2}".format(num_samples, 32,num_cluster)
